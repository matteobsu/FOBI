function fit_res=fit_edges_gauss(spectra,peaks_struct,varargin)
%%   
...
j=[find(x==min(x)):find(x==min(x))+6];
 ...
 %  k=[find(x==max(x))-3:find(x==max(x))]; 
   
     k=[find(x==max(x))-6:find(x==max(x))]; 
...
%             title(sprintf('detector= %i ',l)); 
               ...
               plot(x,yraw'-yfit,'-g'); ylabel('Residual'); xlabel('TOF'); grid on; pause;       
