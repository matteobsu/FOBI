...
nfun = length(npar);
p= pfix; p(find(isnan(pfix)))=pfree;

pi = 1;
F = zeros(size(x));

if nargout == 1 % calculation of J not required
		for ii = 1:nfun % calculate F
   		F = feval(char(fnames(ii)),p(pi:pi-1+npar(ii)),x)+ F;
   		pi = pi + npar(ii);
      end
      ...
      for i = 1:nfun
         ...
         F = F + fi; 
         pi = pi + npar(i);
         J(:,jn:jn+sum(isnan(pfix(pind)))-1) = jp( :,find( isnan(pfix(pind)) ) );
