function pcimg=imgpolarcoord(img,anglebins,radius,angle)
...
%  Authors: Juan Carlos Gutierrez & Javier Montoya.
   ...
   if exist('anglebins','var') == 0
      anglebins = 360;
   end
   abins = deg2rad(linspace(0,angle,anglebins+1)); abins=abins(1:end-1);
   
   pcimg = nan*zeros(length(0:radius),length(abins));
   i     = 1;
